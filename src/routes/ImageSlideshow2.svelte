<script>
  ...
  // Generate the array of .webp image URLs 94 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  const images = [];
  for (let i = 13; i <= 94; i++) {
    images.push(`stimuli/2/webp/${i}.webp`);
  }
 ...
 let themes;
  ...
  function sendToFirebase() {
      ...
      addDoc(collection(db, 'Game-2'), formData)
        .then(() => {
          // Form submission successful, redirect to another page
          window.location.href = 'instruction_break'; // Replace with the desired URL
        })
        .catch((error) => {
